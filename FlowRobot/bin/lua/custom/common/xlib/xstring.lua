...
local sys_lower=string.lower
local sys_upper=string.upper
...
naming=function(str)
	...
	local result_string=""
	for i=1,len,1 do
		local current_char=sys_lower(string.sub(str,i,i))
		local next_char=string.sub(str,i+1,i+1)
		if(not(empty(current_char))) then
			if(not(empty(next_char)) and sys_upper(next_char)==next_char) then
				result_string=result_string..current_char.."_"
			else
